...
from aegis.models.schemas import AgentAction, HttpVerb, SandboxResult


def simulate(action: AgentAction) -> SandboxResult:
    resource_key = (
        f"{action.environment.value}:{action.resource}:{action.verb.value}"
    )
    ...
    if action.verb == HttpVerb.DELETE:
        return SandboxResult(
            ...
            side_effects=[
                "indexes would be invalidated",
                "dependent FK checks would run",
            ],
        )
