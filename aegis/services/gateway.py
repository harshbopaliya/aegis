...
def process_action(action: AgentAction) -> GatewayResult:
    ...
    if reg_block:
        ...
        observability.pipeline_advance(
            request_id, "agent_registry", reg_block.detail
        )
        observability.pipeline_set_status(
            request_id, "blocked_unregistered_agent"
        )
    ...
    observability.pipeline_advance(
        request_id, "sandbox_simulation", sb.predicted_outcome[:280]
    )
    observability.emit(
        "layer_sandbox", request_id, sb.model_dump(), actor_id=actor
    )
    ...
    observability.emit(
        "layer_risk", request_id, risk.model_dump(), actor_id=actor
    )
    ...
    if action.verb != HttpVerb.GET:
        ...
        observability.pipeline_advance(
            request_id, "backup_snapshot", backup_id
        )
        observability.emit(
            "backup_snapshot",
            request_id,
    ...
    observability.pipeline_advance(
        request_id, "execution", "invoking_execution_layer"
    )
...
def resolve_approval(body: ApprovalRequest) -> GatewayResult:
    ok = human_loop.resolve(
        body.request_id, body.approver_id, body.approve, body.reason
    )
    if not ok:
        observability.emit(
    ...
    if not body.approve:
        ...
        observability.pipeline_set_status(
            body.request_id, "rejected_by_human"
        )
        return GatewayResult(
            request_id=body.request_id,
    ...
    if action is None:
        observability.pipeline_set_status(
            body.request_id, "action_expired"
        )
        return GatewayResult(
            request_id=body.request_id,
    ...
    if action.verb != HttpVerb.GET:
        backup_id = backup_recovery.snapshot(
            reason=f"approved_before_{action.verb.value}",
            request_id=body.request_id,
        )
    ...
    observability.pipeline_advance(
        body.request_id,
        "backup_snapshot",
        backup_id or "skipped_read_only",
    )
    observability.pipeline_advance(
        body.request_id, "execution", "post_human_approval"
    )
    ...
    observability.pipeline_set_status(
        body.request_id, "executed_after_approval"
    )
