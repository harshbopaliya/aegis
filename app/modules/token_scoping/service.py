...
from app.models.schemas import AgentAction, HttpVerb, LayerDecision, Role
from app.modules.agents.repository import get as get_agent
...
def evaluate(action_verb: HttpVerb, role: Role) -> LayerDecision:
    ...
    return LayerDecision(
        layer="token_scoping",
        allowed=ok,
        detail="Verb permitted for role." if ok else "Verb not in scope for role (RBAC).",
        metadata={"role": role.value, "verb": action_verb.value},
    )


def resolve_registry(action: AgentAction) -> tuple[AgentAction | None, LayerDecision | None, dict]:
    """
    ...
    """
    meta: dict = {}
    if not settings.require_agent_registration and not settings.enforce_agent_roles_from_registry:
