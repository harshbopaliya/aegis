    ...
    for k, v in d.items():
        ...
        if lk in _REDACT_KEYS or "secret" in lk or "password" in lk:
            if isinstance(v, (dict, list)):
                ...
                out[k] = {"_redacted": True, "sha256": hashlib.sha256(str(v).encode()).hexdigest()}
        elif isinstance(v, dict):
            out[k] = _redact_dict(v, depth + 1)
        elif isinstance(v, list):
            out[k] = [_redact_dict(x, depth + 1) if isinstance(x, dict) else x for x in v[:50]]
        else:
            out[k] = v
    return out


def serialize_for_audit(event_type: str, payload: dict[str, Any]) -> tuple[str, int]:
    """
